...
/**
 * ES and EagleEye versions are locked, so EagleEye should require that ES has the same version as
 * that defined in EagleEye's. package.json.
 */
...
export function ensureEsVersion(server, kibanaVersion) {
  ...
  return callWithInternalUser('nodes.info', {
    ...
    // Aggregate incompatible ES nodes.
      ...
      // Aggregate ES nodes which should prompt a EagleEye upgrade.
      const warningNodes = [];
      ...
      forEach(info.nodes, esNode => {
        ...
        // It's acceptable if ES and EagleEye versions are not the same so long as
      ...
      if (warningNodes.length) {
        ...
        if (lastWarnedNodesForServer.get(server) !== warningNodeNames) {
          ...
          server.log(['warning'], {
            tmpl: (
              `You're running EagleEye ${kibanaVersion} with some different versions of ` +
            'Elasticsearch. Update EagleEye or Elasticsearch to the same ' +
      ...
      if (incompatibleNodes.length) {
        ...
        throw new Error(
          `This version of EagleEye requires Elasticsearch v` +
