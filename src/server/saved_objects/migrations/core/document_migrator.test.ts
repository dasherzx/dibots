...
describe('DocumentMigrator', () => {
  ...
  it('rejects docs that belong to a newer EagleEye instance', () => {
    const migrator = new DocumentMigrator({
      ...testOpts(),
      kibanaVersion: '8.0.1',
    });
    expect(() =>
      ...
      /Document "smelly" has property "dog" which belongs to a more recent version of EagleEye \(10\.2\.0\)/i
    );
  });
  ...
  it('rejects docs that belong to a newer plugin', () => {
    ...
    expect(() =>
      ...
      /Document "fleabag" has property "dawg" which belongs to a more recent version of EagleEye \(1\.2\.4\)/i
    );
  });
