 ...
 * an async generator, as that feature currently breaks EagleEye's. tooling.
 *
 ...
 * some rudimentary expectations. Past EagleEye indices had multiple root documents, etc
 ...
 * check is itself index-agnostic, though the error hint is a bit EagleEye specific.
 *
...
async function assertIsSupportedIndex(indexInfo: FullIndexInfo) {
  ...
  if (isV5Index) {
    throw new Error(
      `Index ${indexInfo.indexName} belongs to a version of EagleEye ` +
