...
interface KibanaFeatures {
  // Indicates whether we can run EagleEye in a so called cluster mode in which
  // EagleEye is run as a "worker" process together with optimizer "worker" process
  // that are orchestrated by the "master" process (dev mode only feature).
  isClusterModeSupported: boolean;

  // Indicates whether we can run EagleEye without X-Pack plugin pack even if it's
